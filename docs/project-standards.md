...
- 제품 목록 검색은 제품명, 제조사, 모델번호를 대상으로 하며 상태 필터와 정렬을
  함께 적용합니다. 검색 결과가 없을 때도 검색 입력은 화면에 유지합니다.
- 공개 리콜 조회는 입력 전, 조회 중, 결과, 결과 없음, 재시도 가능한 오류 상태를
  구분합니다. mock 데이터는 실제 공공 데이터 연동으로 교체할 수 있는 임시 동작입니다.
